...
class ServiceQueue {
  ...
  private:
	  ...
	  struct buzzer
	  {
		  int isBeingUsed = -1; //will be 0 for not being used and 1 for is being used
		  int id = 0; //buzzer number
	  ...
	  struct personQueue
	  {
		  buzzer placeInLine; //to know where the person is in line
		  personQueue* front; //a person knows who's in front of them
		  personQueue* back; //a person knows who's behind them
  ...
  public:
	 ...
	 */
         ServiceQueue() 
		 {
			person = new personQueue;
			person.placeInLine = new buzzer;
			person->front = nullptr;
			person->back = nullptr:
			person.placeInLine.id++;
			person.placeInLine.isBeingUsed = 1;
			person.placeInLine->pPrev = nullptr;
			person.placeInLine->pNext = nullptr;
         }
	...
	int  length() 
	{
		//no ide how to properly do this
		if (person.lineLength != 0)
		{
			return person.lineLength;
		}
	...
	{
		/*
		First need to go through buzzer list and see if there are any buzzers taken

		if(buzzer ISN'T being used so for my case buzzer.isBeingUsed == 0)
		else(buzzer IS being used ignore it)
		*/
             return 0;  // placeholder
    }
