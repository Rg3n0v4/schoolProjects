...
class graph {
    ...
    bool bfs(int src, std::vector<vertex_label> &report) {
      ...
      while(!q.empty()) {
        ...
        for(edge &e : vertices[u].outgoing) 
        {
          ...
          {
            report[v].npaths = report[u].dist + 1;
          }
        }
  ...
  private:
  void enumHelp(int target, string path, int index, vector<string> &paths) 
  {
      int v;
      ...
      {
          //cout << "Hello" << endl;
          paths.push_back(path + " " +  vertices[index].name);
      }
  
      else 
      {
  
        for(edge &e : vertices[index].outgoing) 
        {
