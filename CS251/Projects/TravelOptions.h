...
class TravelOptions{
   ...
   */
    ...
    */          
 					...
 					double priceB, double timeB) 
	{
		...
		if ((priceA != priceB) && (timeA != timeB) && 
				 (priceA < priceB) && (timeA < timeB))
		{
			return better;
		}
		if ((priceB != priceA) && (timeB != timeA) &&
			(priceB < priceA) && (timeB < timeA))
		{
			return worse;
		}
   ...
   */
    ...
    */
    bool is_sorted()const{
		...
		{
			if ((lst->price) > (lst->next->price))
			{
				return false;
			}
			if ((lst->price) == (lst->next->price) && (lst->time) > (lst->next->time))
			{
				return false;
			}
			lst = lst->next;
    ...
    */
    bool is_pareto() const
	{
		...
		while (lst->next != nullptr )
		{
			while (temp != nullptr)
			{
				if (compare(temp, lst) != TravelOptions::incomparable)
				{
					return false;
				}
				temp = temp->next;
			}
			//so that each one is being compared with each other
			lst = lst->next;
			temp = lst->next;
		}
		return true;
    ...
    */
    bool is_pareto_sorted() const
	{
		...
		while (lst->next != nullptr)
		{
			if ((lst->price) > (lst->next->price) && (compare(lst,lst->next) != TravelOptions::incomparable) )
			{
				return false;
			}
			lst = lst->next;
    ...
    bool insert_sorted(double price, double time) {
		...
		while (temp->next != nullptr && temp->next->price < price)
		{
			
			temp = temp->next;
		}
		
		if (temp->next != nullptr && temp->next->price == price)
		{
    ...
    bool insert_pareto_sorted(double price, double time) {
		...
		if (front == nullptr || compare(front, newNode) != TravelOptions::incomparable)
		{
			...
			if (compare(front, newNode) == TravelOptions::better ||
				compare(front, newNode) == TravelOptions::equal)
			{
				delete newNode;
				return true;
			}
			...
			{
				newNode = front;
				while (temp != nullptr && compare(temp, newNode) != TravelOptions::incomparable)
				{
		...
		while (temp->next != nullptr && newNode->price > temp->next->price)
		{
			
			//compare temp and newNode to see if they are incomparable
			temp = temp->next;
		}
		
		if (compare(temp->next, newNode) == TravelOptions::equal ||
			compare(temp->next, newNode) == TravelOptions::better)
		{
		...
		Node* connectingNode;
		connectingNode = temp->next;
		temp->next = newNode;
		while (connectingNode != nullptr && compare(connectingNode, newNode) != TravelOptions::incomparable)
		{
			
			Node* temp2 = connectingNode->next;
			delete connectingNode;
			connectingNode = temp2;
   ...
   */
    TravelOptions * union_pareto_sorted(const TravelOptions &other)const
	{
		...
		firstLst = front;
		Node* otherLst;
		otherLst = other.front;
		TravelOptions * tOptions = new TravelOptions();
		Node* pHead = nullptr;
		Node* endOf_pHead = nullptr;
		
		while (firstLst != nullptr && otherLst != nullptr) //goes until the end of the first list is found
		{
			std::cout << "compare inside while loop" <<compare(firstLst, otherLst)<<std::endl;
			std::cout << "firstLst->price" << firstLst->price << std::endl;
			std::cout << "firstLst->time" << firstLst->time << std::endl;
			std::cout << "otherLst-> price" << otherLst->price << std::endl;
			std::cout << "otherLst-> time" << otherLst->time << std::endl;
			if (compare(firstLst, otherLst) == equal || compare(firstLst, otherLst) == better)
			{
				std::cout << "firstLst->price" << firstLst->price << std::endl;
				std::cout << "otherLst-> price" << otherLst->price << std::endl;
				...
				if (pHead == nullptr)
				{
					...
					std::cout << "otherLst-> price" << otherLst->price << std::endl;
					pHead = new Node;
					pHead->price = firstLst->price;
					pHead->time = firstLst->time;
					pHead->next = nullptr;
					endOf_pHead = pHead;
				}
				else
				{
					Node* temp = new Node;
					temp->price = firstLst->price;
					temp->time = firstLst->time;
					temp->next = nullptr;
					endOf_pHead->next = temp;
					endOf_pHead = endOf_pHead->next;
				}
				firstLst = firstLst->next;
			}
			else if (compare(firstLst, otherLst) == worse)
			{
				std::cout << "firstLst->price" << firstLst->price << std::endl;
				std::cout << "otherLst-> price" << otherLst->price << std::endl;
				firstLst = firstLst->next;
				if (pHead == nullptr)
				{
					...
					std::cout << "otherLst-> price" << otherLst->price << std::endl;
					pHead = new Node;
					pHead->price = otherLst->price;
					pHead->time = otherLst->time;
					pHead->next = nullptr;
					endOf_pHead = pHead;
				}
				else
				{
					Node* temp = new Node;
					temp->price = otherLst->price;
					temp->time = otherLst->time;
					temp->next = nullptr;
					endOf_pHead->next = temp;
					endOf_pHead = endOf_pHead->next;
				}
			...
			else if (compare(firstLst, otherLst) == incomparable)
			{
				std::cout << "firstLst->price" << firstLst->price << std::endl;
				std::cout << "otherLst-> price" << otherLst->price << std::endl;
				if (firstLst->price > otherLst->price)
				{
					std::cout << "firstLst->price" << firstLst->price << std::endl;
					std::cout << "otherLst-> price" << otherLst->price << std::endl;
					Node* temp = new Node;
		...
		if (otherLst == nullptr)
		{
			std::cout << "otherLst is null " << std::endl;
		}
		while (otherLst != nullptr)
		{
			Relationship r = compare(otherLst, endOf_pHead);
			std::cout << "otherLst while firstLst->price "<< otherLst->price << std::endl;
			if (r == incomparable)
			{
		...
		if (firstLst == nullptr)
		{
			std::cout << "firstLst is null " << std::endl;
		}
		while (firstLst != nullptr)
		{
			Relationship r = compare(firstLst, endOf_pHead);
			std::cout << "firstLst while firstLst->price " << firstLst->price << std::endl;
			if (r == incomparable)
			{
				std::cout << "firstLst->price" << firstLst->price << std::endl;
   ...
   */
    bool prune_sorted(){
		...
		while (currentNode->next != nullptr)
		{
			...
			if (compare(currentNode, nextNode) == incomparable)
			{
				prevNode = currentNode;
				currentNode = nextNode;
			}
			else if (compare(currentNode, nextNode) == worse)
			{
   ...
   */
   TravelOptions * join_plus_plus(const TravelOptions &other) const 
   {
	   ...
	   TravelOptions* t_optionsPtr = new TravelOptions();

	   while (firstLst != nullptr)
	   {
		   while (otherLst != nullptr)
		   {
			   t_optionsPtr->insert_sorted(firstLst->price + otherLst->price, firstLst->time, otherLst->time);
			   otherLst = otherLst->next;
		   }
		   firstLst = firstLst->next;
		   otherLst = other.front;
   ...
   */
   TravelOptions * join_plus_max(const TravelOptions &other) const {
	   ...
	   TravelOptions* t_optionsPtr = new TravelOptions();
	   Node* pHead = nullptr;
	   Node* endOfpHead = nullptr;

	   while (firstLst != nullptr && otherLst != nullptr)
	   {
		   Node* temp;
		   temp = new Node;
		   temp->price = firstLst->price + otherLst->price;
		   temp->time = std::max(firstLst->time, otherLst->time);
		   ...
		   {
			   endOfpHead->next = temp;
			   endOfpHead = endOfpHead->next;
		   }

		   if (firstLst->time > otherLst->time)
		   {
			   firstLst = firstLst->next;
		   }
		   else if (otherLst->time > firstLst->time)
		   {
			   otherLst = otherLst->time;
		   }
		   else
		   {
			   firstLst = firstLst->next;
   ...
   */
   TravelOptions * split_sorted_pareto(double max_price) {
	...
	if (firstLst->price > max_price)
	{
		front = prevNode;
		t_optionsPtr->front = firstLst;
		return t_optionsPtr;
	}
	while (firstLst != nullptr && firstLst->price <= max_price)
	{
		prevNode = firstLst;
		firstLst = firstLst->next;
	}
